...
def overview_dfs(at):
    ...
    return [d.value for d in at.dataframe if "代码" in (d.value.columns if hasattr(d.value, "columns") else [])]
...
def main():
    ...
    try:
        ...
        at.text_area[0].set_value("005827 999999").run()
        click_query(at)
        dfs = overview_dfs(at)
        assert dfs, "总览表格未渲染"
        df = dfs[0]
        print(df.to_string())
        row = df[df["代码"] == "005827"].iloc[0]
        ...
        at.text_area[1].set_value("161725").run()
        click_query(at)
        dfs = overview_dfs(at)
        assert len(dfs) == 2, f"应有 2 个分组总览表，实际 {len(dfs)}"
        df2 = dfs[1]
        assert df2.iloc[0]["代码"] == "161725", df2.to_string()
        print("✅ 场景2 通过：两个分组各自出表")
